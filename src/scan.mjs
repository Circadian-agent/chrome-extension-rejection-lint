...
// HALF OF ALL EXTENSION PROJECTS HAVE NO MANIFEST TO READ, and telling those
// developers "no manifest.json in this directory" sends them looking for a file
// that does not exist anywhere yet. Measured, not guessed: of 160 public repos
// in research/data/lint-corpus.json, 90 could not be linted as checked in, and
// of the 84 that were not simply MV2, 45 are a recognisable framework build -
// wxt 18, plasmo 14, crxjs 8, vite-plugin-web-extension 1, plus 8 on bare
// webpack or vite. Our README tells a stranger to run the tool on
// ./my-extension; for these that directory only exists after a build.
//
// THE OUTPUT PATHS ARE READ OFF THE CORPUS, not off memory. Counting the paths
// that appear in .gitignore, README.md and package.json across those repos:
// `.output/chrome-mv3` 8 (wxt, alongside firefox-mv2/edge-mv3/safari-mv3
// siblings) and `build/chrome-mv3-prod` 9 against `build/chrome-mv3-dev` 12 -
// dev is commoner in the corpus but prod is what you submit, so prod is what we
// name.
//
// ORDER MATTERS AND IS NOT ALPHABETICAL: a wxt project also depends on vite, and
// a plasmo project also looks like a bare node project, so the specific
// frameworks are tested before the generic bundlers. The generic entries
// deliberately do NOT name a directory - guessing `dist` for an arbitrary
// webpack config would be the same misdiagnosis one level down.
const FRAMEWORKS = [
  { name: "wxt", files: [/^wxt\.config\.[cm]?[jt]s$/], deps: ["wxt"], build: "npx wxt build", out: ".output/chrome-mv3" },
  { name: "Plasmo", deps: ["plasmo"], build: "npm run build", out: "build/chrome-mv3-prod" },
  { name: "CRXJS", deps: ["@crxjs/vite-plugin"], build: "npm run build", out: "dist" },
  { name: "vite-plugin-web-extension", deps: ["vite-plugin-web-extension", "@samrum/vite-plugin-web-extension"], build: "npm run build", out: "dist" },
  { name: "webextension-toolbox", deps: ["webextension-toolbox"], build: "npm run build", out: "dist/chrome" },
  { name: "webpack", deps: ["webpack"], build: "npm run build", out: null },
  { name: "Vite", deps: ["vite"], build: "npm run build", out: null },
];

// Returns the build hint for a directory that has no manifest.json, or null when
// nothing recognisable is there. Never throws: an unreadable or invalid
// package.json means "no hint", because a crash here would replace a usable
// error message with a stack trace.
export function buildHint(root, exists = existsSync) {
  let pkg = {};
  try { pkg = parseJson(readFileSync(join(root, "package.json"), "utf8")) || {}; } catch { pkg = {}; }
  const deps = { ...(pkg.dependencies || {}), ...(pkg.devDependencies || {}) };
  let names = [];
  try { names = readdirSync(root); } catch { names = []; }

  for (const fw of FRAMEWORKS) {
    const byFile = (fw.files || []).some((rx) => names.some((n) => rx.test(n)));
    const byDep = (fw.deps || []).some((d) => Object.prototype.hasOwnProperty.call(deps, d));
    if (!byFile && !byDep) continue;
    // If the build has already been run, say where it landed rather than telling
    // someone to build again. Only claimed when the manifest is actually there,
    // so this cannot point at a stale empty directory.
    if (fw.out && exists(join(root, fw.out, "manifest.json"))) {
      return `This is a ${fw.name} project and you have already built it. Run: webstore-lint ${fw.out}`;
    }
    return fw.out
      ? `This is a ${fw.name} project, so the manifest is generated by the build and does not exist yet. Run "${fw.build}", then: webstore-lint ${fw.out}`
      : `This looks like a ${fw.name} build, so the manifest is generated and does not exist yet. Run "${fw.build}", then point webstore-lint at your build output directory.`;
  }
  return null;
}
...
export function scan(root) {
  ...
  if (!existsSync(manifestPath)) {
    ...
    // Only for a real directory that simply has no manifest. A path that does
    // not exist, or a file, is a different mistake and a build hint there would
    // be answering a question nobody asked.
    //
    // THIS GUARD IS BELT-AND-BRACES AND THE TESTS DO NOT PROVE IT LOAD-BEARING:
    // removing it fails nothing, because buildHint already returns null for both
    // of those paths (readdirSync and readFileSync both throw ENOTDIR/ENOENT and
    // are caught). That was found by mutating it, not by reading it, and it is
    // recorded here so a later reader does not mistake the passing controls for
    // evidence about this line. The real protection is buildHint's own
    // behaviour, which IS asserted directly. Kept anyway: it costs one statSync
    // field already in hand, and it keeps the intent legible if buildHint is
    // ever made more eager.
    if (st && st.isDirectory()) manifestHint = buildHint(root);
  } else {
    try {
  ...
  return { root, files, skipped, oversized, manifest: localized, manifestRaw: manifest, i18nUnresolved: unresolved, manifestError, manifestHint };
}
